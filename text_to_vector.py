...
def text_to_vector(sentences):
    ...
    得到分词后的句子之后，先得到词表
    每个词对应一个位置
    如果s1出现了一次，就把s1的第一个位置设为1,如果没有出现就是0，
    如果出现了两次，那么s1的第一个位置就是2，以此类推
    """
    ...
    for i in range(len(all_list)):
        ...
        if word_count[word] < min_freq:
            continue
        # 没有出现，那么给他一个id, id正好等于word_to_id的长度e
...
def run_step():
    sentences = readFile(filename='/Users/fan/py/movie_issue/demo_train.txt')
    sentences_valid = readFile(filename='/Users/fan/py/movie_issue/demo_validation.txt')
    # 遍历所有的句子，并打印编号和句子内容
    for i in range(len(sentences)):
    ...
    sentences_splited_valid, labels_valid = split_text(sentences_valid)
    ...
    # 保证train和valid的维度一样的
    all_sentence = sentences_splited + sentences_splited_valid
    train_length = len(sentences_splited)
    onehot_vector = text_to_vector(all_sentence)
    train_vector = onehot_vector[:train_length]
    valid_vector = onehot_vector[train_length:]
    for i in range(len(train_vector)):
        print("index=", i)
        print("source_sentence", sentences_splited[i])
        print("one hot vector", train_vector[i])
        print("label:", labels[i])
        print("===")
